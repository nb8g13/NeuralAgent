...
public class Agent3 extends AbstractNegotiationParty {
	...
	public void init(NegotiationInfo info) {
		...
		this.mlnn2 = new ComputationGraph(this.nn);
		this.firstAgent = true;
		this.buildIndex(info.getUtilitySpace().getDomain());
		
		try {
			this.out = new PrintWriter(new FileWriter("output.txt"));
		} catch (IOException e) {
	...
	public void receiveMessage(AgentID sender, Action act) {
		...
		if (act instanceof Accept) {
			...
			if(this.lastBid != null) {
				...
				INDArray[] previousInputs = buildInput(this.lastBid);
				INDArray[] previousOutput ={new NDArray(previousTarget)};
				MultiDataSet mdsp = new MultiDataSet(previousInputs, previousOutput);
				cmlnn.fit(mdsp);
			}
			
			this.lastBid = offer.getBid();
		}
	...
	public Action chooseAction(List<Class<? extends Action>> list) {
		...
		if (lastBid != null && this.getUtility(lastBid) > this.target) {
			...
			this.out.println("Not exploring");
			double agreementChance = Double.MIN_VALUE;
			double agreement1 = Double.MIN_VALUE;
			double agreement2 = Double.MIN_VALUE;
			...
			do {
				...
				INDArray[] testOutput2 = this.mlnn2.output(testInput);
				
				//this.out.println(testOutput[0].getDouble(0,0));
				double jointProb = testOutput[0].getDouble(0,0)*testOutput2[0].getDouble(0,0);
				
				if(jointProb >= agreementChance) {
					newBid = candBid;
					agreement1 = testOutput[0].getDouble(0,0);
					agreement2 = testOutput2[0].getDouble(0,0);
					agreementChance = jointProb;
					keepLooking = !(agreement1 >= 1.0 && agreement2 > 1.0);
				}
			} while (keepLooking && timeline.getCurrentTime() < start + 3.0);
			
			this.out.println(newBid);
			this.lastBid = newBid;
